...
```text
projects/01_FC換装/params/before_fc_replace/20260607_before_fc_replace.param
projects/01_FC換装/params/after_fc_replace/20260608_pixhawk6c_ardurover_initial.param
projects/02_チューニング/params/tuned/20260610_before_tune.param
projects/02_チューニング/params/tuned/20260610_after_manual_hold_fs_check.param
projects/02_チューニング/params/tuned/20260610_after_acro_speed_check.param
```
...
| 日付 | ファイル | 機体状態 | 概要 |
| --- | --- | --- | --- |
| 2026-06-13 | `projects/02_チューニング/params/tuned/20260613_pixhawk6c_rover_tuned_01.param` | 2026-06-13チューニング終了版 | QuikTune後、通常運用向け。`OA_TYPE=0`。 |
| 2026-06-13 | `projects/02_チューニング/params/tuned/20260613_08_oa_type1_bendyruler_test.param` | BendyRuler実験版 | `OA_TYPE=1`、小回り回避実験用。通常運用正本とは分ける。 |
| 2026-06-13 | `projects/02_チューニング/params/tuned/20260613_06_after_quiktune.param` | QuikTune直後 | `RTun: Tuning DONE` / `tuning gains saved` 後。 |
| 2026-06-10 | `projects/02_チューニング/params/tuned/20260610_before_tune.param` | Pixhawk 6C Mini換装後 / チューニング前 | 現在のチューニング前ベースライン。このファイルは上書きしない。 |
| 2026-06-09 | `projects/01_FC換装/params/after_fc_replace/20260609_pixhawk6c_indoor_manual_ch2_ch5_ok.param` | 屋内Manual確認時 | 2026-06-10版の直前参照。 |
| 2026-06-08 | `projects/01_FC換装/params/after_fc_replace/20260608__pixhawk6c_ardurover_initial.param` | ArduRover初期書き込み後 | 初期状態の参照。 |
| 2026-06-07 | `projects/01_FC換装/params/before_fc_replace/20260607_before_fc_replace.param` | Pixhawk 2.4.8Pro / 換装前 | 換装前の正本。6C Miniへ丸コピーしない。 |

## 2026-06-13 終了時採用値

`projects/02_チューニング/params/tuned/20260613_pixhawk6c_rover_tuned_01.param` から抜粋。
...
`projects/02_チューニング/params/tuned/20260610_before_tune.param` から抜粋。
...
| タイミング | 保存先 | 必ず記録する差分 |
| --- | --- | --- |
| マスト長変更後Compass校正前 | `projects/02_チューニング/params/tuned/20260610_before_compass_mast_cal.param` | 変更前後マスト長、`COMPASS_ENABLE`、`COMPASS_USE*`、`GPS1_TYPE` |
| マスト長変更後Compass校正後 | `projects/02_チューニング/params/tuned/20260610_after_compass_mast_cal.param` | `COMPASS_OFS_*`、`COMPASS_DIA_*`、`COMPASS_ODI_*`、`COMPASS_SCALE`、`COMPASS_ORIENT`、`COMPASS_EXTERNAL`、PreArm結果 |
| 屋外前安全確認後 | `projects/02_チューニング/params/tuned/20260610_after_preoutdoor_check.param` | `COMPASS_ENABLE`、Battery値、Mode割当、RC7確認 |
| Manual / Hold / RC FS確認後 | `projects/02_チューニング/params/tuned/20260610_after_manual_hold_fs_check.param` | `MODE*`、`FS_*`、`SERVO*`、`BATT_*` |
| Acro割当後 | `projects/02_チューニング/params/tuned/20260610_after_acro_assignment.param` | 変更した`MODE*`のみ。Manual / Holdの退避先を明記 |
| Speed確認後 | `projects/02_チューニング/params/tuned/20260610_after_acro_speed_check.param` | `CRUISE_*`、`ATC_SPEED_*`、`ATC_ACCEL_MAX` |
| Turn Rate確認後 | `projects/02_チューニング/params/tuned/20260610_after_turn_rate_check.param` | `ACRO_TURN_RATE`、`ATC_STR_RAT_*`、`ATC_TURN_MAX_G` |
| 初回Auto/Guided確認後 | `projects/02_チューニング/params/tuned/20260610_after_guided_auto_check.param` | `WP_SPEED`、`WP_RADIUS`、`PSC_*`、Compass/GPS関連 |
| 2026-06-13チューニング終了版 | `projects/02_チューニング/params/tuned/20260613_pixhawk6c_rover_tuned_01.param` | QuikTune後の通常運用向け。`OA_TYPE=0` |
| BendyRuler実験版 | `projects/02_チューニング/params/tuned/20260613_08_oa_type1_bendyruler_test.param` | `OA_TYPE=1`。通常運用版とは分ける |
...
- 旧FCの値、候補値、実機現在値を混ぜない。現在値は `projects/02_チューニング/params/tuned/20260610_before_tune.param` 以降のファイルを正とする。

## 変更履歴

| 日付 | 内容 |
| --- | --- |
| 2026-06-13 | QuikTune完了後の通常運用向け終了版 `projects/02_チューニング/params/tuned/20260613_pixhawk6c_rover_tuned_01.param` と、`OA_TYPE=1` 実験版を追記。 |
| 2026-06-10 | `projects/02_チューニング/params/tuned/20260610_before_tune.param` を現在正本として整理。旧候補値との矛盾、次に保存するパラメータ、記録項目を追加。 |
