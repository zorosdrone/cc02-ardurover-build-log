...
Pixhawk 6C Miniへ換装したCC-02の接続先、ケーブルの端子対応、電源系統をまとめます。構成は[部品リスト](../../docs/11_部品リスト.md)と2026-09-05までの配線記録に基づきます。動作確認日と資料更新日は異なり、2026-09-05に全配線を再測定したものではありません。
...
| 系統 | 接続関係 | 記録・確認範囲 |
| --- | --- | --- |
| 走行用主電源 | Zeee 3S LiPo 2200mAh 120C → PM02 / ESC系 | バッテリーに近い太い電源線のXT60を抜いて遮断する。PM02の入力・負荷側、隠れた中継配線は確認待ち。 |
...
| 項目 | 分かっていること | 残る確認 |
| --- | --- | --- |
| 主電源ハーネス | XT60、白色中継コネクタ、太い赤黒線を写真で確認 | PM02入力・負荷側、分岐、極性、ESC電流が計測経路を通るか。全体図では未確定のESC主電源経路を省略。 |
| ステアリングサーボ | DS3218、20kg表記、270°版。搭載者確認。 | 使用品の電圧・停止電流とBEC容量の照合。270°全域を操舵に使わず、リンケージの可動限界に合わせる。 |
| モーター | CC-02キット付属品。詳細型番情報なし。 | 3Sでの使用条件。ESCの3S対応だけでモーター適合性は判断しない。 |
| Pi用モバイルバッテリー | 小型品を別積みする構成 | 現物型番・出力電流定格・給電ケーブル。旧部品表のC25 2500mAhは、現物確認済みの型番ではない。 |
| PM02測定 | Mission Plannerの電圧表示を確認した記録あり | テスター実測との比較、電流表示、負荷時の計測範囲。 |
...
2. 主電源経路を確定し、接続する負荷の仕様に合った電圧と極性を確認する。
