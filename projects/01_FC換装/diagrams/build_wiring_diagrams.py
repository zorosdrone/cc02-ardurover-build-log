...
class Sheet:
    ...
    def save(self,name):
        self.text(45,1135,'2026-09-05 作成｜換装・動作確認済み機体の配線記録｜端子の物理配置・縮尺を示す図ではありません',18,GRAY)
        (OUT/f'{name}.svg').write_text('\n'.join(self.svg+['</svg>']),encoding='utf-8')
        self.im.save(OUT/f'{name}.png')

s=Sheet('01  全体配線図 — CC-02 / ArduRover','基準：Pixhawk 6C Mini Model A + M10 GPS｜青＝信号を含むケーブル、赤＝電源、点線＝詳細経路を省略')
...
s.box(45,650,360,115,'DS3218 / 270°サーボ',['MAIN 1へ3線接続（所有者申告）','ESC内蔵BECから給電'])
...
s.text(55,320,'主電源：実機は接続・動作済み',20,AMBER)
s.text(55,353,'遮断：バッテリー近くのXT60',18,AMBER)
s.text(55,383,'写真で隠れた中継部分は省略',18,AMBER)
s.text(55,420,'※詳細な線の取り回しは図示省略',18,AMBER)
