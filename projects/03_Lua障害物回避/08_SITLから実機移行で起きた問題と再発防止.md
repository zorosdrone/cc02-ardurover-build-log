...
- [使用中の実機パラメータ](../04_WebGCS遠隔操作/params/20260720_01_webapp_rc1_steering_rc3_throttle.param)
- [2026-07-27 Lua試験後パラメータ](params/20260727_01_after_luaoa_v93_avoid_test.param)
...
| v8.1 | 出力設定の事前検査、1.0秒後のPWM到達監視、時間完了と物理動作のログ分離 | 物理後退は成立したが、1.8秒では数cmにとどまり旋回前の離隔が不足した |
| v9 | 後退と操舵を同時出力する案 | SITL版の動作確認に誤認があり、実機投入前に撤回 |
| v9.1 | 直進後退4.5秒、その後に前進旋回4.5秒 | SITL版と同じ状態遷移へ戻し、実機で一応の回避成功を確認 |
| v9.2 | `SCR_USER2～6`による起動中設定、後退2.25秒、旋回4.5秒・40 deg/s | 後退時間を半減し、旋回時間は維持して旋回率を倍増。HOLD／Disarm後の次回Guided開始時に設定を固定 |
| v9.3 | 停止距離1.0 m、走行0.50 m/s、低速0.25 m/s | 実機の接近距離と速度を調整 |
| v9.4相当 | 前進旋回を6.0秒へ延長、後退2.25秒を維持 | 試験後paramは`SCR_USER3=6`。動画を保存。ただしコード内の起動識別子はv9.3のまま |
...
2026-07-27に当日のBINログを解析した結果、`-0.35`中の`RCOU.C3`は約`1448 us`で、速度は概ね`-0.03～+0.03 m/s`にとどまっていた。一方、同じログのManual操作では約`1401 us`以下で明確な負速度が発生し、`1380～1394 us`で約`-0.3～-0.57 m/s`を記録した。

したがって、今回の直接原因は、`-0.35`から生成された約`1448 us`が実機ESCの有効な後退領域へ届かなかったことと判断した。詳細は[2026-07-26 Lua後退失敗 BINログ解析](logs/test_runs/20260726_01_lua_backup_failure_bin_analysis.md)に記録した。

そのためv8.1以降では次を行っている。

- 直接スロットルを`-0.70`へ変更
- `SRV_Channels:get_output_pwm(70)`でThrottle機能の実出力PWMを記録
- BACKUP開始1.0秒後にPWMが`1405 us`以下へ入ったか検査
- PWMを取得できない、または`1405 us`より大きい場合は`FAULT`停止保持

現行パラメータでは、`-0.70`は計算上約`1395 us`となり、今回Manual後退が成立した出力範囲に入る。
v8.1の実機試験では物理後退が成立したため、LuaからESCまでの後退出力経路は機能していると判断できる。
ただし、`BACK_MS=1800`では移動が数cmにとどまり、操舵指令も`0`だったため後退中に方位は変わらなかった。

v9.1では後退出力を`-0.70`のままとし、操舵`0`で`4500 ms`直進後退した。
終了後はSITL版と同じく独立したTURNへ移り、`0.20 m/s`、`20 deg/s`を`4500 ms`間指令した。
この構成で実機の回避に一応成功した。

v9.2では、後退時間の初期値を`2250 ms`へ半減した。TURN時間は`4500 ms`のままとし、旋回率を`20 deg/s`から`40 deg/s`へ倍増した。
さらに`SCR_USER2～6`をMission Plannerから変更可能にした。変更値は回避中には反映せず、HOLDまたはDisarm後、次のGuided開始時に検査して固定する。

v9.3では、停止距離を`2.0 m`から`1.0 m`、走行速度を`0.30 m/s`から`0.50 m/s`、低速値を`0.15 m/s`から`0.25 m/s`へ変更した。直近のv9.4相当変更では、前進旋回時間を`4500 ms`から`6000 ms`へ延長した。現在のコード既定値は次のとおりである。
...
2026-07-27の試験後paramは`SCR_USER1=3`、`SCR_USER3=6`、`SCR_USER2/4/5/6=0`である。`0`はコード既定値を使う指定なので、採用値は後退`2.25秒`、前進旋回`6.0秒`・`0.20 m/s`・`40 deg/s`、後退スロットル`-0.70`となる。
距離と角度を直接計測していないため、時間を半分／2倍にしても実距離と実角度が厳密に比例するとは限らない。

判定方法:

| 現行ログ | 判断 |
| --- | --- |
| `FAULT backup pwm unavailable` | Throttle機能の出力PWMを取得できていない |
| `FAULT backup pwm ... > 1405` | Pixhawk側の出力が実機後退境界へ届かず、自動停止した |
| `back s=0.00/... t=-0.70/...` | BACKUPで操舵中立と後退Throttleが出力されている |
| `turn r=40.0 s=0.20 p=.../...` | TURNで前進速度と旋回率が同時に指令されている |
| `cfg b=2.25 t=6.00 v=0.20` | 今回固定した後退時間、旋回時間、旋回速度 |
| `cfg r=40.0 q=-0.70` | 今回固定した旋回率と後退スロットル |
| `back pwm ok ...; motion unverified`後も車輪が動かない | ESCの後退設定、ニュートラル、配線側を確認 |
| PWMが負方向へ変化し、車輪も逆回転 | LuaからESCまでの出力契約は成立 |
| 車輪は逆回転するが車体が動かない | 駆動系、路面、出力不足を確認 |

### 4. 状態名は動作完了を意味しない

旧版の次のログは、後退が完了したことを意味するように見える。
...
v8.1以降ではログを次のように区別する。

- `back pwm ok ...; motion unverified`: PWM出力契約だけ成立し、物理後退は未確認
- `backup command time complete`: 直進後退の指令時間が終了
- `turn command time complete`: 前進旋回の指令時間が終了
- 物理後退は目視または後続BINログで別に確認
...
現在リポジトリに保存されているSITL用Luaは、コード上は`BACKUP`で直進後退し、その後の`TURN`で前進旋回する。
実機v9.1以降もこの状態遷移へ合わせた。
実機固有の差は、負速度制御ではESCの後退PWM域へ届かなかったため、BACKUPだけを`set_steering_and_throttle(0, -0.70)`へ置き換えている点である。
...
2026-07-28時点で、次は未解決または未確認である。

1. 現行値2.25秒で直進後退距離が適切か
2. 続く6.0秒・40 deg/sの前進TURNで実方位が適切か
3. `SCR_USER2～6`の採用値を起動ログと試験記録で毎回対応付けられるか
4. コミット上のv9.4相当と、コード内`SCRIPT_VERSION=...v9.3`の不一致
5. `RC3_TRIM=1706`と実入力中立付近`1509`の不一致を、物理送信機とWebGCSのどちらを基準に校正するか
6. 対象なし時の0.01～0.02 mをどこまでNO_TARGETとして扱うか
7. 極近距離障害物と対象なしをTF-Luna 1台で区別できるか
8. 後退を時間だけで終了せず、移動確認へ変更するか
...
- [ ] 起動ログでLua版を確認し、コードの`SCRIPT_VERSION`と一致した
