...
現行v9.2では、後退時間の初期値を`2250 ms`へ半減した。TURN時間は`4500 ms`のままとし、旋回率を`20 deg/s`から`40 deg/s`へ倍増した。
...
| `cfg b=2.25 t=4.50 v=0.20` | 今回固定した後退時間、旋回時間、旋回速度 |
...
1. v9.2の初期値2.25秒で直進後退距離が適切か
2. 続く4.5秒・40 deg/sの前進TURNで実方位が適切か
3. Mission Plannerで変更した`SCR_USER2～6`が次回Guided開始ログへ反映されるか
4. `RC3_TRIM=1706`と実入力中立付近`1509`の不一致を、物理送信機とWebGCSのどちらを基準に校正するか
5. 対象なし時の0.01～0.02 mをどこまでNO_TARGETとして扱うか
6. 極近距離障害物と対象なしをTF-Luna 1台で区別できるか
7. 後退を時間だけで終了せず、移動確認へ変更するか
