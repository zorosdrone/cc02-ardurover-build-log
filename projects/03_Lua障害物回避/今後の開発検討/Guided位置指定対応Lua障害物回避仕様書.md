...
> 本書は実装前の初版仕様を残す設計資料であり、表中の初期値は現行実機版の設定値ではない。2026-07-28時点の実装、試験結果、未解決事項は[プロジェクト概要](../README.md)、[実機テスト手順](../07_実機テスト手順.md)、[実機移行の問題と再発防止](../08_SITLから実機移行で起きた問題と再発防止.md)を参照する。
