...
| v9.1 | 直進後退4.5秒、その後に前進旋回4.5秒 | SITL版と同じ状態遷移へ戻し、実機固有差は後退の直接スロットルだけに限定。実機結果確認待ち |
...
現行v9.1では後退出力を`-0.70`のままとし、操舵`0`で直進後退する。
`BACK_MS`は`4500 ms`へ延長し、終了後はSITL版と同じく独立したTURNへ移る。
TURNでは`0.20 m/s`、`20 deg/s`を`4500 ms`間指令し、前進しながら方向を変える。
実際の旋回方向と方位変化量は実機で確認し、方向が逆なら`TURN_DIR`を反転する。
...
| `turn r=20 s=0.20 p=.../...` | TURNで前進速度と旋回率が同時に指令されている |
...
1. v9.1の`back s=0.00/... t=-0.70/...`と`back pwm ok ...`が出るか
2. 4.5秒の直進後退で必要な離隔を確保できるか
3. 続く4.5秒の前進TURNで実方位が十分に変わるか
