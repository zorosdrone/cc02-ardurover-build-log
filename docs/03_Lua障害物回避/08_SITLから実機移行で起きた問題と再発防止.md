...
| v8.1 | 出力設定の事前検査、1.0秒後のPWM到達監視、時間完了と物理動作のログ分離 | `1405 us`へ届かない場合は`FAULT`停止。実機結果確認待ち |
...
現行パラメータでは、v8.1の`-0.70`は計算上約`1395 us`となり、今回Manual後退が成立した出力範囲に入る。
2026-07-27時点では、v8.1のPWM実測と物理後退結果は未確認である。
解決済みとは扱わない。
...
- `backup command time complete`: 指令時間が終了
...
2026-07-27時点で、次は未解決または未確認である。

1. v8.1の`back thr=-0.70`で`back pwm ok ...`が出るか
2. PWMが後退側でもESC／車輪が後退するか
3. `RC3_TRIM=1706`と実入力中立付近`1509`の不一致を、物理送信機とWebGCSのどちらを基準に校正するか
